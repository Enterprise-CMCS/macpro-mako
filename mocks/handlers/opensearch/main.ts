...
const handleMainSearch = async ({ request }: { request: Request }) => {
  ...
  if (regexpQueries) {
    for (const fieldName in regexpQueries) {
      ...
      itemHits = itemHits.filter((item) => {
        const fieldValue =
          item[filteredFieldName] ?? item._source?.[filteredFieldName as keyof typeof item._source];
...
const defaultOSMainAndDraftSearchHandler = http.post<PathParams, SearchQueryBody>(
  "https://vpc-opensearchdomain-mock-domain.us-east-1.es.amazonaws.com/test-namespace-main,test-namespace-draftmain/_search",
  handleMainSearch,
);

const defaultOSMainAndDraftEncodedSearchHandler = http.post<PathParams, SearchQueryBody>(
  "https://vpc-opensearchdomain-mock-domain.us-east-1.es.amazonaws.com/test-namespace-main%2Ctest-namespace-draftmain/_search",
  handleMainSearch,
);
...
export const mainSearchHandlers = [
  ...
  defaultOSMainAndDraftEncodedSearchHandler,
