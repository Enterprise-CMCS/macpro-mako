...
// export const signInHandler = http.post(/\/oauth2\/token/, ({ request }) => {
//   console.log(`\n\nsigning in with request: ${request}`);
...
export const identityProviderServiceHandler = http.post<PathParams, IdpRequestBody>(
  ...
  async ({ request }) => {
    console.log("\n\nidentityProviderServiceHandler called with: ", { request });
...
export const identityServiceHandler = http.post<PathParams, IdentityRequest>(
  /cognito-identity/,
  async ({ request }) => {
    console.log("\n\nidentity service called with: ", { request });
    const target = request.headers.get("x-amz-target");
    if (target) {
      let username;
      const { Logins, IdentityId, IdentityPoolId } = await request.json();
      console.log({ IdentityId, IdentityPoolId, Logins });
      for (const key in Logins) {
        console.log(Logins[key]);
        const payload = jwt.decode(Logins[key]);
      ...
      if (username) {
        ...
        if (target == "AWSCognitoIdentityService.GetId") {
          const encodedId = Buffer.from(
            JSON.stringify({ IdentityId: `us-east-1:${username}` }),
          ).toString("base64");
          ...
          const encodedIdentity = Buffer.from(
            JSON.stringify({
              Credentials: {
                ...
                Expiration: 1.732572918e9,
                SecretKey: SECRET_KEY,
                SessionToken: "",
              },
              IdentityId: `us-east-1:${username}`,
            }),
          ).toString("base64");

          return new HttpResponse(encodedIdentity, {
            headers: {
              "Content-Type": "application/x-amz-json-1.1",
              "Content-Encoding": "base64",
            },
