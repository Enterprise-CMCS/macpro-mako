import type { Attachment, opensearch } from "shared-types";
...
export type ChangelogDocumentTest = Omit<opensearch.changelog.Document, "attachments"> & {
  attachments?: Partial<Attachment>[] | [];
};

export type ChangelogItemResultTest = Partial<opensearch.Hit<Partial<ChangelogDocumentTest>>> & {
  found: boolean;
};

export type MainDocumentTest = Omit<opensearch.main.Document, "changelog"> & {
  changelog?: ChangelogItemResultTest[] | [];
};

export type MainItemResultTest = Partial<opensearch.Hit<Partial<MainDocumentTest>>> & {
  found: boolean;
};

const items: Record<string, MainItemResultTest> = {
  [EXISTING_ITEM_ID]: {
