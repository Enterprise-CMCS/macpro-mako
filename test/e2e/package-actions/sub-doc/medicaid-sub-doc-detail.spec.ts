...
test.describe("UI - Validation", () => {
  test.describe("Breadcrumbs", () => {
    test("should displays breadcrumb elements", async ({ page }) => {
      ...
      await expect(page.locator("#root > div > main > div:nth-child(2) > nav")).toHaveText(
        "DashboardCO-22-2020New Subsequent Documentation",
      );
    });
  });
  ...
  test.describe("Form Elements", () => {
    ...
    test("Document Section", async ({ page }) => {
      ...
      await expect(page.getByTestId("attachment-section-title")).toHaveText(
        "Subsequent Medicaid SPA Documents *",
      );
      ...
      await expect(page.getByTestId("attachments-instructions")).toHaveText(
      ...
      await expect(page.getByTestId("accepted-files")).toHaveText(
    ...
    test("Reason Section", async ({ page }) => {
      ...
      await expect(page.getByTestId("additional-info-title")).toHaveText(
