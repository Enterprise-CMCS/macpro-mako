...
test.describe("User Profile", { tag: ["@smprod"] }, () => {
  ...
  test.describe("UI Validations", () => {
    ...
    test("Status", async ({ page }) => {
      ...
      await expect(page.getByRole("heading", { name: "Status" })).toHaveText("Status");
    });
  });
  ...
  test.describe("Interactions", () => {
    ...
    test("navigate to Role change page", async ({ page }) => {
      ...
      await expect(page.getByRole("heading", { name: "CMS Role Approver" })).toBeVisible();
      await expect(page.getByRole("heading", { name: "CMS Role Approver" })).toHaveText(
        "CMS Role Approver",
      );
    });
  });
