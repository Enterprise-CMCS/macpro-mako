...
test.describe("dashboard smoke test - prod", { tag: ["@smprod"] }, () => {
  ...
  test.describe("UI validations", () => {
    ...
    test("page header", async ({ page }) => {
      ...
      await expect(dashboardPage.submissionBtn).not.toBeVisible();
    });
  ...
  test.describe("Interactions", () => {
    test("Download Exports", async ({ page }, testInfo) => {
      const meta = getMetadataByFile(testInfo.file!);

      if (meta) {
        testInfo.annotations.push({ type: "test:id", description: meta.id });
        testInfo.annotations.push({ type: "test:title", description: meta.title });
