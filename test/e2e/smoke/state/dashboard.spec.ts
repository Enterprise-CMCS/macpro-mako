...
test.describe("dashboard smoke test - prod", { tag: ["@smprod"] }, () => {
  ...
  test.describe("UI validations", () => {
    ...
    test("page header", async ({ page }) => {
      ...
      await expect(dashboardPage.submissionBtn).toBeVisible();
      await expect(dashboardPage.submissionBtn).toHaveText("New Submission");
    });
  ...
  test.describe("Interactions", () => {
    test("Download Exports", async ({ page }, testInfo) => {
      ...
      if (meta) {
        testInfo.annotations.push({ type: "meta:id", description: meta.id });
