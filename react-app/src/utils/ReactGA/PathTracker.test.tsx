...
describe("PathTracker", () => {
  ...
  it("sends a page_view event on initial mount", () => {
    render(<PathTracker userRole="cms"><div>Test Page</div></PathTracker>);
  ...
  it("tracks route changes via pushState", () => {
    render(<PathTracker userRole="state"><div>Test Page</div></PathTracker>);
    ...
    expect(sendGAEventMock).toHaveBeenNthCalledWith(1, "page_view", expect.objectContaining({
      page_path: "/",
      referrer: "/",
      user_role: "state",
    }));
    ...
    expect(sendGAEventMock).toHaveBeenNthCalledWith(2, "page_duration", expect.objectContaining({
      page_path: "/",
      time_on_page_sec: 3,
      user_role: "state",
    }));
    ...
    expect(sendGAEventMock).toHaveBeenNthCalledWith(3, "page_view", expect.objectContaining({
        page_path: "/new-path",
        referrer: "/", // the prevPathRef has already been updated
        user_role: "state",
    }));
  ...
  it("sends page_duration on unmount", () => {
    const { unmount } = render(<PathTracker userRole="cms"><div>Test Page</div></PathTracker>);
    vi.advanceTimersByTime(4000);

    unmount();

    expect(sendGAEventMock).toHaveBeenCalledWith("page_duration", expect.objectContaining({
      page_path: window.location.pathname,
      user_role: "cms",
      time_on_page_sec: 4,
    }));
  ...
  it("ignores route change if pathname does not change", () => {
    ...
    // Should not call sendGAEvent again
    expect(sendGAEventMock).toHaveBeenCalledTimes(1); // only initial page_view
  });
});
