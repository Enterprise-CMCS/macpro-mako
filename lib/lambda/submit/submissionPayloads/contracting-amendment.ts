...
export const contractingAmendment = async (event: APIGatewayEvent) => {
  if (!event.body) return;

  const parsedResult = events["contracting-amendment"].baseSchema.safeParse(
  ...
  const transformedData = events["contracting-amendment"].schema.parse({
    ...parsedResult.data,
    submitterName,
    submitterEmail,
    timestamp: Date.now(),
