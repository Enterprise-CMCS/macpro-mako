...
export const newChipDetailsSubmission = async (event: APIGatewayEvent) => {
  if (!event.body) return;

  const parsedResult = events["new-chip-details-submission"].baseSchema.safeParse(
  ...
  const transformedData = events["new-chip-details-submission"].schema.parse({
    ...parsedResult.data,
