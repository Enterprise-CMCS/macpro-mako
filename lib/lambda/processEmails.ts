...
if (!REGION || !EMAIL_LOOKUP_SECRET_NAME || !APPLICATION_ENDPOINT_URL || !OPENSEARCH_DOMAIN_ENDPOINT || !INDEX_NAMESPACE) {
  throw new Error("Environment variables are not set properly.");
}
...
export const handler: Handler<KafkaEvent> = async (event) => {
  try {
    const processRecordsPromises = Object.values(event.records).flat().map((rec) =>
      processRecord(rec, EMAIL_LOOKUP_SECRET_NAME, APPLICATION_ENDPOINT_URL)
    );
...
function createEmailParams(filledTemplate: any, sourceEmail: string): SendEmailCommandInput {
  return {
    Destination: {
    ...
    Message: {
      Body: {
        ...
        Text: filledTemplate.text ? { Data: filledTemplate.text, Charset: "UTF-8" } : undefined,
      },
