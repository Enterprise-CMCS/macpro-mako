...
export type FetchPackageOptions = {
  ...
  allowHiddenSmartReservations?: boolean;
...
const defaults: FetchPackageOptions = {
  allowNotFound: false,
  allowHiddenSmartReservations: false,
  setToContext: false,
  rethrowErrors: false,
 ...
 * @param {boolean} opts.allowHiddenSmartReservations [false] if true, return hidden SMART reservations for occupancy checks
...
export const fetchPackage = (opts: FetchPackageOptions = {}): MiddlewareObj => {
  ...
  return {
    before: async (request: Request) => {
      ...
      if (
        packageResult?.found &&
        !options.allowHiddenSmartReservations &&
        isHiddenSmartReservation(packageResult._source)
      ) {
